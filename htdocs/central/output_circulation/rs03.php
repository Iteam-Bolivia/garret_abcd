...
$Sort="V15,v20";
$Pft="v15`$$$`V20`$$$`V10 `$$$`V60".'`$$$`f(mfn,1,0)`$$$`,@'.$Pft;
...
foreach ($contenido as $value){	$value=trim($value);	if ($value!=""){		$v=explode('$$$',$value);
		...
		$Control_no=$v[1];
		$arrHttp["usuario"]=$v[2];
		$fecha_asignacion=$v[3];
		if ($bd.$Control_no!=$key_comp){
			if ($key_comp!=""){				echo "</form>";
			}
			//SE DETERMINA SI LA BASE DE DATOS TIENE COPIAS			$key_comp=$bd.$Control_no;
			$tit_comp=$bd." ".$Control_no;
			if (isset($lo_database[$bd])){				$copies="Y";
			}else{				$copies="N";			}
			echo "<form name=$key_comp method=post action=rs03_ex.php>";
			echo "<tr><td colspan=$ixcols bgcolor=white align=center><p><font color=darkred><strong>$tit_comp ".$msgstr["total_copies"]."</font></strong>";
			echo "<table bgcolor=#dddddd cellpadding=3>
			<th>".$msgstr["inventory"]."</th>
			<th>".$msgstr["usercode"]."</th>
			<th>".$msgstr["devdate"]."</th>";
			$disponibles=BuscarEjemplaresDisponibles($bd,$Control_no,$copies);
			echo "<tr><td>".$msgstr["disp_loan"].": <font color=red>$disponibles</font>";
            echo "</table>".$msgstr["reserves"]."</td></tr><tr>";
		...
		if ($vencidos==0 and $total_susp==0 and $total_multa==0 and $disponibles>0 and $fecha_asignacion=="" )
			echo "&nbsp;<a href=javascript:AssignReserve(".$Mfn.")><img src=../dataentry/img/add.gif alt='".$msgstr["assign_copy"]."' title='".$msgstr["assign_copy"]."'></a>";
		if ($fecha_asignacion!="") $disponibles=$disponibles-1;
		echo "</td>";
		echo "</tr>";
	}}
if ($key_comp!=""){

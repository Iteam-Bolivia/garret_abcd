...
function PrestamoMismoObjeto($control_number,$user){
global $copies_title,$msgstr,$obj;
	$msg="";	$tr_prestamos=LocalizarTransacciones($control_number,"ON");
...
global $xWxis,$Wxis,$db_path,$msgstr,$wxisUrl;
	...
	foreach ($contenido as $value){
		if (trim($value)!=""){
			...
			if (isset($r[4]) and $r[4]!="" and $usuario==$r[1]){
...
// se lee la configuración de la base de datos de objetos de préstamos
	...
	// se verifica si el ejemplar está prestado
					$tr_prestamos=LocalizarTransacciones($arrHttp["inventory"],"TR");
					$Opcion="";
					$msg="";
					$msg_1="";
					...
					$var=PrestamoMismoObjeto($control_number,$arrHttp["usuario"]);
					$msg=$var[0];
	        			...
	        			$ec_output.="<td bgcolor=white valign=top>";	        			if ($grabar=="Y"){
	        				...
	        				if ($reservado[0]=="continuar"){
	        					...
	        					if (isset($tpp[$tipo_obj]))
	        						...
	        						$tpp[$tipo_obj]=1;
	        					//echo  "<p>".$np. "<". $tprestamos_p ." and ". $tpp[$tipo_obj]." <=".$tprestamos_politica ."<br>";
...
function ShowReservations(CN,BD){	msgwin=window.open("../reserve/show_reservations.php?submenu=N&key="+CN+"&bases="+BD,"reservations","width=1000,height=400,resizable")
	msgwin.focus()}

...
// se presenta la  información del usuario
	...
	if (isset($arrHttp["reserve"])){		$ec_output.="<table width=100% border=0><td>";
	}
	foreach ($contenido as $linea){		$ec_output.= $linea."\n";
	}
	if (isset($arrHttp["reserve"])){
		$ec_output.="</td><td align=right valign=top>";
		$ec_output.="<a href=\"javascript:Reservar(".$arrHttp["usuario"].")\">
							<img src=\"../images/icon/reservar.png\" border=0 alt=\"".$msgstr["reserve"]."\" title=\"".$msgstr["reserve"]."\" />
						</a>";
		...
		foreach ($prestamos as $linea) {
			if (trim($linea)!=""){
				...
				if (isset($p[17]) and trim($p[17])!=""){					$politica_este=explode('|',$p[17]);
					...
					$politica_str=$politica[$p[3]][$p[6]];				}
                $lapso_p=$politica_este[5];
                if (isset( $total_politica[$politica_este[0]]))
                	$total_politica[$politica_este[0]]= $total_politica[$politica_este[0]]+1;
                else
                	$total_politica[$politica_este[0]]=1;
				...
				$xnum_p++;
				$ec_output.="$xnum_p. <input type=checkbox name=chkPr_".$xnum_p." value=$mora  id='".$p[0]."'>";
				...
				$ec_output.="</td>
					...
					"</td><td bgcolor=white nowrap align=center valign=top>".$p[12]."(".$p[13].")</td><td bgcolor=white valign=top>".$p[2]."</td><td bgcolor=white align=center valign=top>". $p[3]. "</td><td bgcolor=white nowrap align=center valign=top>".$p[4]."</td><td nowrap bgcolor=white align=center valign=top>$fuente";
					 if ($lapso_p=="D"){					 	$ixd=strpos($p[5]," ");
					 	if ($ixd>0)
					 		...
					 		$ec_output.=  $p[5];					 }else{					 	$ec_output.=  $p[5];					 }
					 $ec_output.=  "</td><td align=center bgcolor=white valign=top>".$mora." ".$lapso_p."</td><td align=center bgcolor=white valign=top>". $p[11]."</td></tr>\n";
        	}
		}
		$ec_output.= "</table>\n";

...
foreach ($fp as $value){
	...
	if ($value!=""){		$b=explode('|',$value);
		$archivo="";
		if (file_exists($db_path.$b[0]."/loans/".$_SESSION["lang"]."/loans_store.pft"))			$archivo=$db_path.$b[0]."/loans/".$_SESSION["lang"]."/loans_store.pft";
		else
		    if (file_exists($db_path.$b[0]."/loans/".$lang_db."/loans_store.pft"))
		    	$archivo=$db_path.$b[0]."/loans/".$lang_db."/loans_store.pft";
...
echo "</td>";
echo "<tr><td valign=top align=right><h2><a href=javascript:Continuar()>".$msgstr["enable"]."</a> &nbsp; &nbsp" ;

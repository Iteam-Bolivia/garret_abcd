		...
		case "S":
			...
			switch (bd){
				...
				default:
					...
					prefijo=b[2]
					formato=b[3]
					AbrirIndiceAlfabetico(xI,prefijo,"","",bd,bd+".par","3",1,"1",formato)
					break
			}
			break		case "I":
			Separa=""
			...
			if (search_in==""){
    			Prefijo=Separa+"&prefijo=IN_"
				url_indice="capturaclaves.php?opcion=autoridades&base=loanobjects&cipar=loanobjects.par"+Prefijo+"&postings=1"+"&lang="+lang+"&repetible=1&Formato=@autoridades.pft"
			}else{
				ix=document.inventorysearch.db_inven.selectedIndex
				sel=document.inventorysearch.db_inven.options[ix].value
				s=sel.split('|')
				bd=s[0]
				pref="IN_"				Prefijo=Separa+"&prefijo="+pref
				url_indice="capturaclaves.php?opcion=autoridades&base="+bd+"&cipar="+bd+".par"+Prefijo+"&postings=1"+"&lang="+lang+"&repetible=1&Formato=@autoridades.pft"			}
			break
		case "U":
<?php

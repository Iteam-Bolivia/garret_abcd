...
function CheckDate(input){
	...
	if ($config_date_format=="DD/MM/YY")
		echo "fecha=f[2]+\"/\"+f[1]+\"/\"+f[0]\n";
	else
        echo "fecha=f[2]+\"/\"+f[0]+\"/\"+f[1]\n";
?>
	var validformat=/^\d{4}\/\d{2}\/\d{2}$/ //Basic check for format validity
	var returnval=false
	if (!validformat.test(fecha))
		alert("<?php echo $msgstr["dateformat_inv"]?>")
...
function EnviarForma(){	returnval=CheckDate(document.inventorysearch.fecha_dev)
	if (returnval==false){
		document.inventorysearch.fecha_dev.select()
		return
	}
...
<div class="middle list">
	...
	<table>
		<td width=100 valign=top>
			<strong>
			<?php echo $msgstr["devdate"]?>
			</strong>
		</td>
		<td valign=top>
			<input type=text name=fecha_dev size=10 maxlength=10 value=<?php
			if ($config_date_format=="DD/MM/YY")
				echo date("d/m/Y");
			else
				echo date("m/d/Y");
...
if (isset($arrHttp["devuelto"]) and $arrHttp["devuelto"]=="S" and isset($arrHttp["rec_dev"])){
	$lista_mfn=explode(';',$arrHttp["rec_dev"]);

...
if (isset($arrHttp["Formato"]))
	echo "Formato='".urlencode($arrHttp["Formato"])."'\n";
else
	echo "Formato=''\n";
	echo "
    	db='".$arrHttp["base"]."'
		...
		URL='alfa.php?&opcion=autoridades&base='+db+'&cipar='+cipar+'&pref='+Pref+'&prefijo='+Prefijo+capturar+'&formato_e=".urlencode($arrHttp["formato_e"])."'+cnvtabsel
		if (Formato!=\"\"){			URL+='&Formato='+Formato		}
...
<?php
	...
	foreach ($contenido as $linea){
		...
		//	if (substr($f[1],0,strlen($arrHttp["pref"]))!=$arrHttp["pref"]) break;
			echo "<option value=".$f[1]."title='".$f[0]."' alt='".$f[0]."'>".$f[0];
		}
	}

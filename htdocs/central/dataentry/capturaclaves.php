...
global $arrHttp,$Formato,$xWxis,$Wxis,$db_path;
...
if (isset($arrHttp["tesauro"])){	$arrHttp["base"]=$arrHttp["tesauro"];
	...
	if ($cisis_ver=="unicode/"){
  	...
  	$query = "&base=".$arrHttp["base"] ."&cipar=$db_path"."par/".$arrHttp["cipar"]."&Opcion=autoridades"."&tagfst=".substr($arrHttp["tagfst"],3)."&prefijo=".$Prefijo."&pref=".$Pref."&postings=".$arrHttp["postings"]."&formato_e=".urlencode($Formato).$ver;
	//echo $query;
	$IsisScript=$xWxis."ifp.xis";
	include("../common/wxis_llamar.php");

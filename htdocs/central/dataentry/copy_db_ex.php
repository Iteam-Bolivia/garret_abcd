...
echo "
	...
	echo "<font color=white>&nbsp; &nbsp; Script: copy_db_ex.php</font>";
	echo "
...
if (isset($arrHttp["reorganize"])){
	...
	if (substr($mx_path,strlen($mx_path)-1,1)!="/")
		$mx_path.="/";
	$res=exec($mx_path."mx $from create=$to -all now tell=1 ",$contenido,$resultado);
	//$res=exec($mx_path."crunchmf $from $to  target=$target ",$contenido,$resultado);
	$fp=fopen($to.".$extension","w");
	//fwrite($fp,$mx_path."crunchmf $from $to  target=$target  ");
	fwrite($fp,$mx_path."mx $from create=$to -all now tell=1");
	fclose($fp);
	//echo "<font color=red>CISIS: <B>".$mx_path."crunchmf $from $to  target=$target</B></font><br>";
	echo "<font color=red>CISIS: <B>".$mx_path."mx $from create=$to -all now tell=1</B></font><br>";

	...
	if (isset($fp)) {
		foreach($fp as $linea){
			if (trim($linea)!="") {
				...
				$nom=trim($l[1]);
				$avail_wks=0;
				if (isset($_SESSION["permiso"]["CENTRAL_ALL"]) or isset($_SESSION["permiso"][$arrHttp["base"]."_fmt_ALL"]) or $avail_wks==1){
					$i=$i+1;
					$wks_p[$cod]="Y";
				}
			}
...
global $arrHttp,$db_path,$xWxis,$Wxis,$valortag,$tl,$nr,$Mfn,$wxisUrl,$lang_db,$msgstr,$registro,$Expresion;

	$Expresion=stripslashes($arrHttp["Expresion"]);
	$Expresion=str_replace('\"','"',$Expresion);
//	echo "<font face=arial style=font-size:10px>".$msgstr["expresion"].": ".$Expresion."<br>";
...
//echo "</xmp>";
	...
	case "buscar":
        ...
        if ($resultado=="0"){        	$arrHttp["Opcion"]=="ninguna";
	        ...
	        die;
	        break;        }else{
        	if ($arrHttp["Formato"]!="")include("toolbar_record.php");
        	//if ($wxisUrl!="") echo $wxisUrl."<br>";
...
//se lee la fdt de la base de datos
	if ($arrHttp["Opcion"]=="crear" or $arrHttp["Opcion"]=="capturar") {		if (isset($_SESSION["valdef"])){			$fp=explode("\n",$_SESSION["valdef"]);
			foreach ($fp as $linea){
				...
				if (!isset($valortag[$tag])) $valortag[$tag]=substr($linea,4);
			}
